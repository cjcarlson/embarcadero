...
#' @description
#'
#' Partial dependence plots show the response curves of an individual variable in the sum-of-trees models. The lines are taken as the average of each posterior draw of sum-of-trees models, and is generated by evaluating the curve at each specified x value for *each other combination of other x values in the data*; this is obviously computationally very expensive. With more smooth you add, the more variables you ask for, and more posterior draws (defaults to 1000) in the bart() function, this will get slower. 
#'
